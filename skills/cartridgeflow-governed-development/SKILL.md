...
当前技能版本：`1.0.3`。以同目录 `VERSION` 为机器可读版本。
...
6. **只在任务 worktree 编辑**：遵循目标仓既有模式。产品和 DR 不得读取、导入、启动、记录或要求治理存在。未发布协议代际不得进入实现；先核对产品协议锁、运行时能力和消费者。作者端准备暴露一个选项前，必须先核对最终消费者真实支持范围；生产者能保存不等于消费者能装载或运行。
7. **先跑目标仓原生证明**：从最小相关测试开始，再扩大到构建、协议审计和 conformance。UI 必须做真实浏览器行为与布局验证；含令牌、签名、下载或安装的流程必须从真实页面按钮走到成功结果，不能用直接调用后端代替。传输层、日志层和脱敏层不得改写成功业务响应；只在展示或记录边界生成脱敏副本。跨仓合同必须拿生产者实际生成的交付物进入真实消费者，完成安装、读取或运行中的目标行为，不以生产者单测、静态包结构或模拟消费者代替。
