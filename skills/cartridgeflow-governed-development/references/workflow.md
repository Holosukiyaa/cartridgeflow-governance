...
跨仓交付必须保存一条可复现的消费者证据链：记录生产者提交、生成包名与摘要、消费者提交、实际导入结果和目标行为结果。若作者端新增组件、协议版本或配置选项，至少用一次真实产物走到最终消费者；只验证作者端预览、序列化结果或 ZIP 内容都不算完成。

涉及验证令牌、签名摘要、下载地址或安装请求时，必须同时证明“接口直接调用成功”和“真实 UI 操作成功”。成功响应是业务合同，不得被通用脱敏、日志或遥测适配器原地改写；需要输出到界面或日志时，对副本脱敏。若脚本成功而 UI 失败，按消费者场景缺陷处理，不得用脚本结果判定完成。

## 5. 治理验收

最后一次目标仓代码、锁文件或生成物变化后，先重建并验证索引。若后续原生验证又修了产品代码，必须再次重建，不能复用修复前的索引：
